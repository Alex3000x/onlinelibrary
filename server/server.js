...
const ACCESS_SECRET =
  "4a8f9c2d1b5e7f3a0d6c8b9a2f4e1d7c3b0a9f8e7d6c5b4a3f2e1d0c9b8a7f6e";
const REFRESH_SECRET =
  "d9e8f7a6b5c4d3e2f1a0b9c8d7e6f5a4b3c2d1e0f9a8b7c6d5e4f3a2b1c0d9e8";
  ...
  try {
    ...
    if (!req.user.isAdmin) {
      return res
        ...
        .json({
          error:
            "Accesso negato. Solo gli amministratori possono aggiungere libri.",
        });
    }
    ...
    const validatedCover = await checkOpenLibraryCover(
      req.body.cover,
      req.body.ISBN,
    );
    ...
    const newBook = {
      ...
      publicationYear:
        isNaN(Number(req.body.publicationYear)) ||
        Number(req.body.publicationYear) <= 0
          ? null
  ...
  try {
    ...
    if (!req.user.isAdmin) {
      return res
        ...
        .json({
          error:
            "Accesso negato. Solo gli amministratori possono eliminare libri.",
        });
    }
    ...
    res.status(500).json({
      error:
        "Si è verificato un errore interno durante l'eliminazione del libro.",
    });
  }
});
  ...
  try {
    ...
    if (!req.user.isAdmin) {
      return res
        ...
        .json({
          error:
            "Accesso negato. Solo gli amministratori possono modify libri.",
        });
    }
    ...
    res.status(500).json({
      error:
        "Si è verificato un errore interno durante l'aggiornamento del libro.",
    });
    next(err);
  }
});
...
app.post("/topinibrary/register", async (req, res, next) => {
  try {
    ...
    const newUser = {
      ...
      avatarUrl:
        req.body.avatarUrl ||
        "https://cdn-icons-png.flaticon.com/512/1144/1144760.png", // Imposta un avatar di default se non fornito
...
app.put("/topinibrary/users/:id", verifyToken, async (req, res, next) => {
  try {
    ...
    if (req.user.id !== req.params.id && !req.user.isAdmin) {
      return res
        .status(403)
        .json({ error: "Accesso negato. Non puoi modificare questo profilo." });
    }
  ...
  async (req, res, next) => {
    try {
      ...
      if (emailExists) {
        return res
          .status(409)
          .json({ error: "Questo indirizzo email è già in uso" });
      }
...
app.get("/topinibrary/users", verifyToken, async (req, res, next) => {
  try {
    ...
    if (!req.user.isAdmin) {
      return res
        .status(403)
        .json({ error: "Accesso negato. Area riservata agli amministratori." });
    }
...
// ==========================================================
app.patch(
  "/topinibrary/admin/users/:id/role",
  verifyToken,
  async (req, res, next) => {
    try {
      if (!req.user.isAdmin) {
      ...
      if (req.user.id === req.params.id) {
        return res
          .status(400)
          .json({ error: "Non puoi revocare i permessi admin a te stesso." });
      }
