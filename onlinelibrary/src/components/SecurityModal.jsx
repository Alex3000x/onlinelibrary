...
function SecurityModal({ show, onHide, user, onLogout, onUpdateSuccess }) {
  ...
  // Reset dei form quando il modale si chiude
  const handleClose = () => {
  ...
  const handlePasswordChange = async (e) => {
    ...
    try {
      await axios.patch(
        `http://localhost:3000/topinibrary/users/${user.id}/password`,
        passwords,
        {
          headers: {
  ...
  const handleEmailChange = async (e) => {
    ...
    try {
      ...
      const response = await axios.patch(
        `http://localhost:3000/topinibrary/users/${user.id}/email`,
        emailData,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("accessToken")}`,
      ...
      alert(
        err.response?.data?.error ||
          "Errore durante l'aggiornamento dell'email",
      );
    }
  };
  ...
  const handleDeleteAccount = async () => {
    if (
      ...
      try {
        await axios.delete(
          `http://localhost:3000/topinibrary/users/${user.id}`,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("accessToken")}`,
            },
  ...
  return (
    <Modal show={show} onHide={handleClose} centered>
      <Modal.Header closeButton className="border-0">
        <Modal.Title className="fw-bold" style={{ color: "#e190b6" }}>
          🔒 Sicurezza Account
        </Modal.Title>
      </Modal.Header>
      ...
      >
        ...
        <h6 className="fw-bold mb-3" style={{ color: "#1f1612" }}>
        ...
        <Form onSubmit={handleEmailChange} className="mb-3">
          ...
          <Button
            ...
            style={{ borderColor: "#e190b6", color: "#e190b6" }}
          >
            Aggiorna Email
          </Button>
        </Form>
        ...
        <h6 className="fw-bold mb-3 mt-3" style={{ color: "#1f1612" }}>
          Cambia Password
        </h6>
        ...
        <div
          className="mt-4 p-3 border border-danger-subtle rounded"
          style={{ backgroundColor: "#fff5f5" }}
        >
          <h6 className="text-danger fw-bold">Zona Pericolo</h6>
          <p className="small text-muted">
            L'eliminazione dell'account prevede una cancellazione definitiva dei
            tuoi dati personali e quelli relativi al tuo account. Procedere
            comunque?
          </p>
          <Button
            variant="danger"
